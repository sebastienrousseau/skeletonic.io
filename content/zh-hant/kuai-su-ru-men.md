...
剛接觸 Skeletonic Stylus？在 **v2.0.0** 中，按需選用功能比以往更加簡單。得益於極小的體積（核心樣式表 gzip 後約 7.9&nbsp;KB），它可以快速整合至任何 Web 應用程式——無需 JavaScript 框架，如果使用 CDN 則連建置步驟也不需要。
...
| 檔案 | 用途 | 大小（min/gz） |
|---|---|---|
| `core/skeletonic.min.css` | 重設 + token + 版面 + 元素 + 組件 + 工具類別 | 42.3 KB / 7.9 KB |
...
**gzip 後的大小是多少？**
壓縮後 42.3&nbsp;KB，**gzip 後約 7.9&nbsp;KB**，brotli 後約 6.8&nbsp;KB（完整核心樣式表）。8&nbsp;KB 上限在 CI 中強制執行。
...
<script type="application/ld+json">
{
  ...
  "mainEntity": [
    ...
    },
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "壓縮後 42.3 KB，gzip 後約 7.9 KB，brotli 後約 6.8 KB（完整核心樣式表）。8 KB 上限在 CI 每次提交時強制執行。"
      }
    },
    {
      "@type": "Question",
